<%-- 
    Document   : view_specific_statistic_outcome
    Created on : 21 May 2026, 11:40:37 PM
    Author     : Ryan
--%>
...
<html>
    <head>
        ...
        <title>View Specific Statistic Outcome Page</title>
    </head>
    <body>
        <h1>View Specific Statistic Outcome</h1>
        <%
            String compName = pageContext.getServletContext().getInitParameter("comp_name");
            String nameSess = (String)session.getAttribute("name");
        %>
        <p>
            <b><%=nameSess%></b>, below is the requested game statistic:
        </p>
        <table border='1'>
        <%
                Statistics s = (Statistics)request.getAttribute("statistic");
                Long id = s.getId();
